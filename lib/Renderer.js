...
function copyAndInitGraph(graph) {
  if (graph == null) throw new Error("graph is null");
  var copy = graph.copy();
...
function createNodeRoots(graph, svg) {
  var roots = (svg == null) ? this._nodeRoots : svg.selectAll('g.node');
...
function createEdgeRoots(graph, svg) {
  var roots = (svg == null) ? this._edgeRoots : svg.selectAll('g.edge');
...
function getLine(layout, dimensions) {
  ...
  if ('BT' === rankDir) {
    getY = function (d) { return dimensions.height - d.y; };
  } else {
    getY = function (d) { return d.y };
  }
  if ('RL' === rankDir) {
    getX = function (d) { return dimensions.width - d.x };
  } else {
...
function reposition(graph, svgNodes, svgEdges, dimensions) {
  ...
  this._transition(svgNodes)
    .attr('transform', function(u) {
      ...
      if ('BT' === rankDir) {
        return 'translate(' + value.x + ',' + (dimensions.height - value.y) + ')';
      } else if ('RL' === rankDir) {
        return 'translate(' + (dimensions.width - value.x) + ',' + value.y + ')';
      } else {
  ...
  this._transition(svgEdges)
    ...
    .attr('transform', function(e) {
      ...
      if ('BT' === rankDir) {
        return 'translate(' + point.x + ',' + (dimensions.height - point.y) + ')';
      } else {
...
function defaultPostRender(graph, root) {
  if (root == null) { return; } // Do not call on update.
