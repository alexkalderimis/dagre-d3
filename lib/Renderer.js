...
Renderer.prototype.layout = function(layout) {
  if (!arguments.length) { return bind(this._layout, this); }
  this._layout = layout;
  return this;
};
...
Renderer.prototype.run = function(graph, svg) {
  ...
  drawEdgeLabels.call(this, graph, this._drawEdgeLabel, svgEdges);
  ...
  drawEdges.call(this, result, this._drawEdge, svgEdges);
...
function defaultDrawNode(graph, u, root) {
  ...
  var label = root.append('g')
                  .attr('class', 'label');
  var labelContent = renderer._getNodeLabel(graph.node(u));
  addLabel(labelContent, label, 10, 10);
}
